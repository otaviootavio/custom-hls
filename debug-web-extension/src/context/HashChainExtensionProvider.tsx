...
import { HashChainElementSchema, SecretLengthSchema, HashObjectSchema } from "../utils/zod-schemas";
import { z } from "zod";

export type HashChainElement = z.infer<typeof HashChainElementSchema>;
export type SecretLength = z.infer<typeof SecretLengthSchema>;
export type HashObject = z.infer<typeof HashObjectSchema>;
...
interface HashChainExtensionContextType {
  ...
  fetchSecretAndLength: () => Promise<SecretLength>;
...
const HashchainFromExtensionContext = createContext<HashChainExtensionContextType | undefined>(undefined);

export const HashChainExtensionProvider: React.FC<HashChainExtensionProviderProps> = ({ children }) => {
  const [hashChainElements, setHashChainElements] = useState<HashChainElement[]>([]);
  ...
  const fetchAndPopHashFromHashChain = async (): Promise<HashChainElement> => {
  ...
  const fetchTail = async (): Promise<string> => {
    ...
    const response = await createEventPromise<{ type: string; data: string }>("Recover_h(100)");
  ...
  const fetchHashChain = async (): Promise<string[]> => {
    ...
    const response = await createEventPromise<{ type: string; data: string[] }>("fullHashChain");
    setFullHashChain(response.data);
    return response.data;
  };

  const fetchSecretAndLength = async (): Promise<SecretLength> => {
    ...
    try {
      ...
      const validatedResponse = SecretLengthSchema.parse(response.data);
  ...
  const syncLastHashSendIndex = async (lastHashSendIndex: number): Promise<number> => {
    window.postMessage(
