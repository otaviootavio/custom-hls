...
interface HashchainContextType {
  ...
  updateHashchain: (hashchainId: HashchainId, updateData: Partial<HashchainData>) => Promise<void>;
...
export const HashchainProvider: React.FC<{ children: React.ReactNode }> = ({ 
  ...
  const [selectedHashchain, setSelectedHashchain] = useState<SelectedHashchain | null>(null);
  ...
  const refreshHashchains = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch hashchains'));
    } finally {
      setLoading(false);
    }
  }, [repository]);
  ...
  const createHashchain = useCallback(async (
  ...
  const selectHashchain = useCallback(async (
    hashchainId: HashchainId | null
  ): Promise<void> => {
  ...
  const syncHashchainIndex = useCallback(async (
    hashchainId: HashchainId, 
    newIndex: number
  ): Promise<void> => {
  ...
  const updateHashchain = useCallback(async (
    hashchainId: HashchainId, 
  ...
  const importHashchain = useCallback(async (
    data: ImportHashchainData
  ): Promise<HashchainId> => {
